"""`CombinerTrainer` — the shared training logic for a `CombinerDetector`.

Fits the parameters for two of `CombinerDetector`'s methods:
...
* **``method="meta"``** — ``fit_meta_classifier()`` trains a small tree-based
  ...
  the member score vector.

``compare_methods()`` fits both and scores ``max`` / ``weighted`` / ``meta`` (and
each bare member) side by side on a held-out split.

Subclasses set ``detector_cls`` (which `CombinerDetector` subclass ``as_detector``
builds) and ``_label`` (for progress/log messages), and usually add a
``use_default()`` that constructs the members.
"""
...
from shared_types.training import ClassifierTrainableModel, LabeledImageSample, TrainingResult

from detector_common.base import _roc_auc
from detector_common.combiner import CombinerDetector
...
def _grid_size(ticks: int, n: int) -> int:
    """Number of points on the simplex grid = C(ticks + n - 1, n - 1)."""
    return math.comb(ticks + n - 1, n - 1)


def _effective_ticks(n: int, step: float, max_points: int = 8000) -> int:
    """``round(1/step)`` ticks, coarsened until the simplex grid fits
    ``max_points`` (the full grid blows up combinatorially past ~4 members —
    for many members prefer ``method="meta"``)."""
    ticks = max(1, int(round(1.0 / step)))
    while ticks > 2 and _grid_size(ticks, n) > max_points:
        ticks -= 1
    return ticks


def _weight_grid(n: int, step: float, *, max_points: int = 8000) -> Iterable[tuple[float, ...]]:
    """Normalized weight tuples over the (n-1)-simplex on a ``step`` grid,
    auto-coarsened so the grid never exceeds ``max_points``."""
    ticks = _effective_ticks(n, step, max_points)
...
def _threshold_candidates(scores: Sequence[float], step: float, *, per_sample: bool = True) -> list[float]:
    if not scores:
        return [0.5]
    lo, hi = min(scores), max(scores)
    grid = [i * step for i in range(int(1.0 / step) + 1)]
    if per_sample and len(scores) <= 2000:
...
def _pick_threshold(
    ...
    *,
    ...
    candidates = _threshold_candidates(scores, step, per_sample=per_sample)
    ...
    if best is None:  # nothing met the FPR budget — fall back to the lowest-FPR point
        best = min((_metrics_at(scores, y, t) for t in candidates),
...
class CombinerTrainer(ClassifierTrainableModel):
    """Fits a `CombinerDetector`'s weights / meta-classifier on labelled data."""

    name = "combiner-trainer"
    #: which CombinerDetector subclass ``as_detector`` builds
    detector_cls: type[CombinerDetector] = CombinerDetector
    #: prefix for progress bars / log lines
    _label = "combiner"
    ...
    @property
    def _max_default_threshold(self) -> float:
        return float(getattr(self.detector_cls, "default_max_threshold", 0.5))
    ...
    def member_score_matrix(
        ...
        and pass them back into ``optimal_weights(X=, y=)`` /
        ``fit_meta_classifier(X=, y=)`` / ``compare_methods(X_train=, ...)`` to
        re-search cheaply.
        """
        if not self._members:
            raise RuntimeError(f"no members — build with {type(self).__name__}.use_default(...) or pass members=")
        ...
        try:
            ...
            iterator = tqdm(samples, desc=f"{self._label}: member scores", unit="img")  # type: ignore[arg-type]
        except Exception:
            pass
    ...
    def optimal_weights(
        ...
        *,
        ...
        objective: ``"balanced_accuracy"`` (default), ``"accuracy"``, ``"f1"``,
            ``"youden"`` (TPR-FPR), or ``"auc"`` (threshold-free).
        max_fpr: if set, only (weights, threshold) with FPR <= this are
            considered, maximizing recall among them.
        weight_step / threshold_step: grid resolution. The weight grid is
            auto-coarsened to at most ``max_weight_points`` (it grows
            combinatorially with the member count — for many members prefer
            ``fit_meta_classifier``). ``top`` weight vectors are returned as
            ``ranked``.

        Stores ``weights_`` / ``threshold_`` / ``metrics_``.
        """
        ...
        ticks = _effective_ticks(n_members, weight_step, max_weight_points)
        grid = list(_weight_grid(n_members, weight_step, max_points=max_weight_points))
        if ticks < round(1.0 / weight_step) and n_members > 3:
            print(f"[{self._label}] optimal_weights: {n_members} members -> weight grid "
                  f"coarsened to step {1.0 / ticks:.3f} ({len(grid)} points). "
                  "For many members fit_meta_classifier() usually does better.")

        # coarse pass: cheap threshold search per weight vector
        coarse_step = max(threshold_step, 0.02)
        ...
        for weights in grid:
            ...
            if objective == "auc" and max_fpr is None:
                ...
                op = _pick_threshold(fused, y, objective="balanced_accuracy",
                                     max_fpr=None, step=coarse_step, per_sample=False)
            else:
                op = _pick_threshold(fused, y, objective=objective, max_fpr=max_fpr,
                                     step=coarse_step, per_sample=False)
                key = op["recall"] if max_fpr is not None else op[objective]
            scored.append((key, weights, op))

        scored.sort(key=lambda item: item[0], reverse=True)

        # fine pass: re-tune the threshold for the best few weight vectors
        for i, (key, weights, _op) in enumerate(scored[: max(top, 10)]):
            fused = [_fuse(row, weights) for row in X]
            op = _pick_threshold(fused, y, objective=objective, max_fpr=max_fpr, step=threshold_step)
            new_key = (op["recall"] if max_fpr is not None
                       else _roc_auc([s for s, t in zip(fused, y) if t == 1],
                                     [s for s, t in zip(fused, y) if t == 0])
                       if objective == "auc" else op[objective])
            scored[i] = (new_key, weights, op)
        scored.sort(key=lambda item: item[0], reverse=True)
        ...
        self.metrics_ = {k: v for k, v in result.items() if isinstance(v, (int, float))}
        return result
    ...
    def fit_meta_classifier(
        ...
        **estimator_kwargs: Any,
        ...
        kind: ``"tree"`` (default, dependency-free CART), ``"sklearn-tree"``,
            ``"forest"``, ``"gboost"`` (HistGradientBoosting), or ``"xgboost"``.
            Ignored if ``estimator=`` (any ``fit`` / ``predict_proba`` object) given.
        feature_spec: ``"probs"`` (raw member p(ai)) / ``"augmented"`` (also
            max / min / mean / abs_diff / product) / a custom list.
        tune_threshold: sweep the meta output for the best ``objective`` (or best
            recall subject to ``max_fpr``); else the threshold stays 0.5.

        Stores ``meta_`` / ``meta_threshold_``; returns feature importances, tree
        ``rules`` and ``train`` (+ ``val``) metric dicts.
        """
    ...
    def train(self, samples: Iterable[LabeledImageSample], **kwargs: Any) -> TrainingResult:
        """Fit the member weights + threshold (thin wrapper over ``optimal_weights``)."""
    ...
    def evaluate(
        ...
        **kwargs: Any,
        ...
        """Score one method on ``samples`` (or a precomputed ``X, y``).

        ``method``: ``"max"`` (no fitting needed) / ``"mean"`` / ``"weighted"``
        (needs ``optimal_weights``) / ``"meta"`` (needs ``fit_meta_classifier``).
        ...
        if method in ("max", "mean"):
            ...
            default_thr = self._max_default_threshold if method == "max" else 0.5
    ...
    def compare_methods(
        ...
        *,
        ...
        Read-only: restores the trainer's fitted state afterwards. Returns
        ``{method_name: metric_dict}``. ``meta_kinds`` you don't have installed
        ...
        if X_val is None or y_val is None:
            if val_samples is not None:
                ...
                print(f"[{self._label}] compare_methods: no val split — scoring on the "
                      "train split (optimistic).")
        ...
        # bare members
        names = self.member_names or [f"member{i}" for i in range(len(X_train[0]))]
        ...
        results["max@default"] = _classify_report(max_va, y_val, self._max_default_threshold)
            ...
            for spec in feature_specs:
                ...
                try:
                    rep = self.fit_meta_classifier(
                        ...
                        objective=objective, max_fpr=max_fpr, X_val=X_val, y_val=y_val,
                    )
                except ImportError as error:
                    print(f"[{self._label}] skipping {tag}: {error}")
                    continue
                results[tag] = rep["val"]
    ...
    def save(self, path: str | Path) -> None:
        """JSON manifest at ``path`` + ``<path>.meta.pkl`` if a meta classifier is
        fitted. Members are not saved (rebuild via ``use_default``)."""
        path = Path(path)
    ...
    @classmethod
    def load(cls, path: str | Path, *, members: Sequence[Any] | None = None) -> "CombinerTrainer":
        ...
        if blob.get("has_meta"):
            from detector_common.meta import MetaClassifier
    ...
    def as_detector(
        ...
        **detector_kwargs: Any,
        ...
        """Build a `detector_cls` instance from what's been fitted.
        ...
        if not use_members:
            raise RuntimeError(f"no members — pass members= or build via {type(self).__name__}.use_default")

        if method == "meta":
            if self.meta_ is None:
                raise RuntimeError("no meta classifier — call fit_meta_classifier() first")
            det = self.detector_cls(use_members, method="max", **detector_kwargs)  # dodge the meta guard
            det.attach_meta_classifier(
                self.meta_,
        ...
        if method == "max":
            return self.detector_cls(
                use_members, method="max", decision_threshold=decision_threshold, **detector_kwargs,
            )
