...
class YoloTrainer(ClassifierTrainableModel):
    """Extend/instantiate this in your Colab notebook:
        ...
        trainer.save("yolo.pt")

    Or just run this module for a small streamed smoke test:
        python -m yolo.trainer --train-per-label 25 --epochs 5
    """
    ...
    @classmethod
    def load(cls, path: str | Path) -> "YoloTrainer":
        ...
        ``YoloDetector.from_checkpoint(path, positive_class=...)``
...
# --- CLI smoke run ------------------------------------------------------
# `python -m yolo.trainer` (or running this file directly)
...
def _run_cli(argv: list[str] | None = None) -> None:
    ...
    trainer = YoloTrainer(base_weights=args.base_weights, image_size=args.image_size)
