...
def resolve_ai_scorer(
    ...
    raise ValueError(
        ...
        "Pass positive_class=<class name or int index> to YoloDetector "
...
def load_yolo_model(
    ...
    *,
    ...
    if missing or unexpected:
        print(f"[yolo] loaded {path.name}: {len(missing)} missing, {len(unexpected)} unexpected keys")
...
class YoloDetector(EnsembleDetector):
    """Wrap any Ultralytics YOLO *classification* checkpoint as a detector.

        from yolo import YoloDetector

        # trained by YoloTrainer (classes: real / ai_generated)
        detector = YoloDetector.from_checkpoint("yolo.pt")

        # someone else's checkpoint with different class names
        detector = YoloDetector.from_checkpoint(
    ...
    DEFAULT_CHECKPOINT = SCRIPT_DIR.parent / "weights" / "yolo_augmented.pt"
    ...
    def use_default(
        ...
        *,
        ...
        (``src/weights/yolo_augmented.pt``).
        """
        if checkpoint is not None:
            checkpoint = Path(checkpoint).expanduser()
        else:
            checkpoint = locate_checkpoint(
                ("yolo_augmented.pt", "yolo_augmented*.pt", "yolo_cls*.pt", "yolo*aigc*.pt"),
                script_dir=SCRIPT_DIR, env_var="YOLO_CHECKPOINT",
            )
        if checkpoint is None or not checkpoint.is_file():
            looked = ", ".join(str(d) for d in candidate_weight_dirs(
                SCRIPT_DIR, env_var="YOLO_CHECKPOINT"))
            raise FileNotFoundError(
                "YOLO classifier checkpoint (yolo_augmented.pt) not found. "
                "Pass checkpoint=<path>, set $YOLO_CHECKPOINT, or drop the "
    ...
    def evaluate(
        ...
        **kwargs: Any,
        ...
        Mirrors `YoloTrainer.evaluate()`'s calling convention:
